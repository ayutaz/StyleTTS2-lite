...
| 入力 | 出力 | N変異体 |
|------|------|---------|
| こんにちは | `k o n n i ʧ i w ɑ` | N_n（nの前） |
| にほん | `n i h o ɴ` | N_uvular（語末） |
| さんぽ | `s ɑ m p o` | N_m（pの前） |
| りんご | `ɾ i ŋ ɡ o` | N_ng（gの前） |
| 東京 | `t o o k j o o` | - |
...
| 入力 | 出力 |
|------|------|
| Hello、こんにちは | `hʌloʊ , k o n n i ʧ i w ɑ` |
...
#### N変異体（文脈依存・piper-plus準拠）
| 内部表現 | IPA | 出現条件 |
|----------|-----|----------|
| N_m | m | m/b/pの前（両唇同化） |
| N_n | n | n/t/d/ts/chの前（歯茎同化） |
| N_ng | ŋ | k/gの前（軟口蓋同化） |
| N_uvular | ɴ | 語末・母音前（口蓋垂鼻音） |
...
- **長音**: 長母音マーカー（:）対応済み。一部の単語では連続母音として出力される場合あり
- **アクセント**: `use_prosody=True`で韻律マーク抽出可能。`convert_with_prosody()`でA1/A2/A3情報取得可能
- **無声化**: `keep_unvoiced_markers=True`で無声化記号（̥）を保持可能
